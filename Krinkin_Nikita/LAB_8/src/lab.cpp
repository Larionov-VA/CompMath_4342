...
int Jacobi(int N, double** A, double* F, double* X, double eps) {
    ...
    do {
        ...
        iterations++;
        if (fabs(X[0]) < 1e-20 || fabs(X[0]) > 1e20) {
            std::cerr << "Error: unsolvable!\n";
            break;
        }
...
int main() {
    ...
    std::cin >> N;
    std::cout << "Enter precision (decimal places): ";
    std::cin >> pr;
    auto A = createMatrix(N);
    auto vectors = getCorrectResponseVectors(A, N);
    double* JacobiAnswer = new double[N]();
    int iter = Jacobi(N, A, vectors.second, JacobiAnswer, std::pow(10, -pr));
    std::cout << "Solution found! Iterations -- " << iter << std::endl;
    for (int i = 0; i < N; i++) {
        std::cout << "X[" << i << "] = " << JacobiAnswer[i]
                  << " (correct " << vectors.first[i] << ")\n";
    }
