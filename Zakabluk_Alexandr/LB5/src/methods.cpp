...
extern double F(double x) {
    // Вариант 5: f(x) = arctg(1/x) - x^2
    return atan(1.0 / x) - (x * x);
}
// Добавляем вычисление первой производной для метода Ньютона
extern double F1(double x) {
    // Производная: f'(x) = -1 / (x^2 + 1) - 2x
    return (-1.0 / (x * x + 1.0)) - (2.0 * x);
}
#else
extern double F(double x) {
    return Round(atan(1.0 / x) - (x * x), DELTA);
}
// Производная с учетом погрешности (округления)
extern double F1(double x) {
